...
"""Canonical match-equity table (MET) shipped with gvformat.

Kazaross-XG2 MET data (both the pre-Crawford table and the post-Crawford
vector) is taken from GNU Backgammon's ``met/Kazaross-XG2.xml``:
...
This is the *single* MET used for every equity<->MWC conversion across all
sources (XG, BGF, and bgsage-powered analysis). Shipping one table is what lets
the format store only cubeful equities: MWC is a *compute-on-read* quantity,
derived from ``(equity, away1, away2, cube_value, is_crawford)`` anywhere the
score and cube are known -- so the format needs no per-decision MWC anchors.

The table is **Kazaross-XG2** (``KR_XG2_MET``), the modern de-facto standard
(eXtreme Gammon's default MET), a 25x25 pre-Crawford table indexed by
``(away1 - 1, away2 - 1)`` giving the on-roll player's match-winning chance at
the start of a game before the cube is turned.

``eq2mwc`` is exactly affine in equity for a fixed ``(score, cube)``, so the two
anchors -- the mover's MWC on a win (equity +1) and on a loss (equity -1) of the
current cubeful game -- linearize any equity on that ply with no engine:

    mwc = (mwc_win + mwc_lose) / 2 + equity * (mwc_win - mwc_lose) / 2

The JavaScript mirror is ``gvformat-js/src/met.js`` -- keep the table and this
formula byte-for-byte identical.
"""

from __future__ import annotations

# ---------------------------------------------------------------------------
# Kazaross-XG2 MET (pre-Crawford table, 25x25). Indexed [away1-1][away2-1].
# ---------------------------------------------------------------------------
...
# POST_CRAWFORD[k-1] = match-winning chance of the player who is k-away when the
# opponent is 1-away (post-Crawford), the k-away side being the trailer. The
# even/odd wobble encodes the leader's free-drop. A proper MET is two pieces:
# the pre-Crawford table above (both players >= 2 away), used when winning/losing
# doesn't put anyone at 1-away, and this vector, used the moment a score resolves
# to 1-away. GNUbg never reads the pre-Crawford table's 1-away row/column for a
# real lookup -- those cells are recursion scratch, not post-Crawford-consistent.
# ---------------------------------------------------------------------------
...
def _me_pre(x: int, y: int) -> float:
    """Resolved-score match equity for a *pre-Crawford* ply: an ``x``-away
    player vs a ``y``-away opponent. ``x <= 0`` -> that player won (1.0); ``y <=
    0`` -> the opponent won (0.0). A 1-away score uses the pre-Crawford table's
    1-away row/column, which holds the **Crawford-game** value -- reaching 1-away
    from a pre-Crawford score means the next game is the (dead-cube) Crawford
    game, not post-Crawford."""
    if x <= 0:
        return 1.0
    if y <= 0:
        return 0.0
    return KR_XG2_MET[min(x, _N) - 1][min(y, _N) - 1]


def _me_post(x: int, y: int) -> float:
    """Resolved-score match equity in the *post-Crawford* regime (the Crawford
    game has already been played, so the cube is live and the trailer redoubles
    at once). A 1-away score routes to the post-Crawford vector -- ~DMP-like
    values, distinct from the Crawford-game values in the pre-Crawford row."""
    if x <= 0:
        return 1.0
    if y <= 0:
        return 0.0
    x = min(x, _N)
    y = min(y, _N)
    if x == 1:            # this player is the 1-away leader
        return 1.0 - POST_CRAWFORD[y - 1]
    if y == 1:            # opponent is the 1-away leader; this player trails
        return POST_CRAWFORD[x - 1]
    return KR_XG2_MET[x - 1][y - 1]


def mwc_anchors(away1: int, away2: int, cube_value: int, is_crawford: bool = False) -> tuple[float, float]:
    """Return ``(mwc_on_win, mwc_on_loss)`` for the mover (``away1``-away) at
    this score/cube.

    ``mwc_on_win`` is the mover's match-winning chance if they win the current
    cubeful game outright (scoring ``cube_value``); ``mwc_on_loss`` if they lose
    it (opponent scores ``cube_value``). Which resolver a resolved score uses
    depends on the *current* ply's regime:

      * **pre-Crawford** (both sides >= 2 away): resolved scores use the
        pre-Crawford table; reaching 1-away lands on the Crawford-game value in
        its 1-away row/column.
      * **Crawford** (``is_crawford``, cube dead so effective cube = 1) and
        **post-Crawford** (a side already 1-away, cube live): resolved scores are
        past the Crawford game, so they use the post-Crawford vector.

    Money game (either side already home / non-positive away count) degenerates
    to the trivial (1.0, 0.0) anchors.
    """
    if away1 <= 0 or away2 <= 0:
        return (1.0, 0.0)
    if is_crawford or away1 == 1 or away2 == 1:
        cube = 1 if is_crawford else cube_value  # Crawford game: dead cube
        me = _me_post
    else:
        cube = cube_value
        me = _me_pre
    mwc_win = me(away1 - cube, away2)          # mover scored `cube`
    mwc_lose = 1.0 - me(away2 - cube, away1)   # opponent scored `cube`
    return (mwc_win, mwc_lose)


def eq2mwc(equity: float, away1: int, away2: int, cube_value: int, is_crawford: bool = False) -> float:
    ...
    return (mwc_win + mwc_lose) / 2.0 + equity * (mwc_win - mwc_lose) / 2.0


def mwc2eq(mwc: float, away1: int, away2: int, cube_value: int, is_crawford: bool = False) -> float:
    """Inverse of :func:`eq2mwc`: an MWC back into this ply's equity frame.

    Equity 0 sits at the *midpoint* of this game's win/loss anchors, not at the
    current MWC -- the two coincide only when the score is symmetric. Converting
    an MWC from outside the ply (a resolved score, another game) therefore needs
    this rather than a bare subtraction. Degenerate anchors give 0.0.
    """
    mwc_win, mwc_lose = mwc_anchors(away1, away2, cube_value, is_crawford)
    span = mwc_win - mwc_lose
    if span == 0.0:
        return 0.0
    return 2.0 * (mwc - (mwc_win + mwc_lose) / 2.0) / span


def score_mwc(away1: int, away2: int, is_crawford: bool = False) -> float:
    """The mover's MWC at a score, *before* the game at that score is played.

    The regime test is deliberately not :func:`mwc_anchors`'. That one resolves
    the score reached *after* this game, so it routes a Crawford ply through the
    ...
    if not is_crawford and (away1 == 1 or away2 == 1):
        return _me_post(away1, away2)   # post-Crawford: cube live
    return _me_pre(away1, away2)
